...
CREATE TABLE IF NOT EXISTS project_attachments (
  ...
  project_id UUID NOT NULL REFERENCES improvement_projects(id) ON DELETE CASCADE,
...
CREATE POLICY "Users can view their own project attachments" ON project_attachments
  FOR SELECT USING (
    user_id = auth.uid() OR 
    project_id IN (
      SELECT id FROM improvement_projects WHERE user_id = auth.uid()
    )
  );
...
CREATE INDEX IF NOT EXISTS idx_project_attachments_user_id ON project_attachments(user_id);
